            ...
            when (token) {
                ...
                is Token.Operator -> {
                    while (opStack.isNotEmpty() &&
                        precedence(opStack.last().op) >= precedence(token.op)
                    ) {
                        output.add(opStack.removeLast())
                    }
                    opStack.addLast(token)
                }
                Token.LeftParen -> opStack.addLast(Token.Operator('('))
                Token.RightParen -> {
                    while (opStack.isNotEmpty() && opStack.last().op != '(') {
                        output.add(opStack.removeLast())
                    }
                    if (opStack.isEmpty() || opStack.last().op != '(') {
